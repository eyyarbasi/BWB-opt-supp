...
vsp_file = os.path.join(os.path.dirname(__file__), "BWB.vsp3")
...
#)
surfaces = generate_vsp_surfaces(
    vsp_file, symmetry=True, include=["BWB"]       # "Tail"
)
...
prob.driver = om.ScipyOptimizeDriver(maxiter=3)
prob.driver.options["debug_print"] = ["nl_cons", "objs", "desvars"]

recorder = om.SqliteRecorder("BWB.db")
...
prob.model.add_design_var("BWB.twist_cp", lower=-10, upper=10.0)
prob.model.add_constraint(point_name + ".BWB_perf.CL", equals=0.5)
prob.model.add_objective(point_name + ".BWB_perf.CD", scaler=1e4)
...
print("CL", prob["flight_condition_0.BWB_perf.CL"][0])
print("CD", prob["flight_condition_0.BWB_perf.CD"][0])