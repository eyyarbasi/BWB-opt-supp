...
vsp_file = os.path.join(os.path.dirname(__file__), "BWB_5.vsp3")
# mesh_dict = {"num_y": 5, "num_x": 5, "wing_type": "CRM", "symmetry": True, "num_twist_cp": 7}

# mesh, twist_cp = generate_mesh(mesh_dict)

surfaces = generate_vsp_surfaces(vsp_file, symmetry=False, include=["BWB"])  # "Tail"
wb = generate_vsp_surfaces(vsp_file, symmetry=False, include=["Fuel"])
wb_mesh = wb[0]["mesh"]


def generate_wb_mesh(surfaces):
    # docs checkpoint 0
    # -----------------------------------------------------------------------------
    ...
    vsp_mesh = surfaces[0]["mesh"]

    half_span = 50  # wing half-span in m
    kink_location = 20  # spanwise location of the kink in m

    root_chord = 20  # root chord in m
    kink_chord = 10  # kink chord in m section 19
    tip_chord = 4.28  # tip ch

    inboard_LE_sweep = 0.0  # inboard leading-edge sweep angle in deg  52.0 deg def
    outboard_LE_sweep = 27.0  # outboard leading-edge sweep angle in deg

    # Mesh specifications

    # nx = 5  # number of chordwise nodal points (should be odd)
    # ny_outboard = 7  # number of spanwise nodal points for the outboard segment
    # ny_inboard = 5  # number of spanwise nodal points for the inboard segment

    nx, ny_outboard, ny_inboard = vsp_mesh.shape[0], 11, 11
    ...
    x_LE[
        ny_inboard : ny_inboard + ny_outboard
...
wb_mesh = generate_wb_mesh(surfaces)
# Data for wingbox definition
# fmt: off

upper_x = np.linspace(0.2,0.7,51,dtype = "complex128")
lower_x = np.linspace(0.2,0.7,51,dtype = "complex128")
...
# np.arange(0.3, 0.7, 41)

# Parameters from FLOPS
cruise_mach = 0.87
...
surf_dict = {
    ...
    # "mesh": mesh,
    "twist_cp": np.array([6.0, 7.0, 7.0, 8, 8, 7, 7, 6]),
    ...
    "skin_thickness_cp": np.array(
        [0.003, 0.006, 0.010, 0.012, 0.012, 0.01, 0.006, 0.003]
    ),  # [m]
    ...
    "t_over_c_cp": np.array(
        [0.074, 0.075, 0.087, 0.1, 0.13, 0.15, 0.15, 0.13, 0.1, 0.087, 0.075, 0.074]
    ),
    ...
    "mrho": 1.9e3,  # [kg/m^3] material density def 2.78e3 (alum)
    "strength_factor_for_upper_skin": 1.0,  # the yield stress is multiplied by this factor for the upper skin
    "wing_weight_ratio": 0.5,  # def 1.25
    ...
    "wingbox_mesh": wb_mesh,
}
#! For point masses use:  "n_point_masses": 0,  # number of point masses in the system
# surfaces = [surf_dict]
...
indep_var_comp.add_output(
    "v", val=np.array([0.87 * 310.95, 0.3 * 340.294]), units="m/s"
)
indep_var_comp.add_output("alpha", val=0.0, units="deg")
indep_var_comp.add_output("alpha_maneuver", val=0.0, units="deg")
indep_var_comp.add_output("Mach_number", val=np.array([0.87, 0.3]))
indep_var_comp.add_output(
    "re",
    val=np.array(
        [
            0.569 * 310.95 * 0.8 * 1.0 / (1.56 * 1e-5),
            1.225 * 340.294 * 0.3 * 1.0 / (1.81206 * 1e-5),
        ]
    ),
    units="1/m",
)
indep_var_comp.add_output("rho", val=np.array([0.569, 1.225]), units="kg/m**3")
indep_var_comp.add_output("CT", val=0.43 / 3600, units="1/s")  # 0.43
indep_var_comp.add_output("R", val=2e6, units="m")
indep_var_comp.add_output(
    "W0", val=29626.7 * 0.45 + surf_dict["Wf_reserve"], units="kg"
)  # OEW 296268*
indep_var_comp.add_output(
    "speed_of_sound", val=np.array([310.95, 340.294]), units="m/s"
)
indep_var_comp.add_output("load_factor", val=np.array([1.0, 2.25]))
indep_var_comp.add_output("empty_cg", val=np.zeros((3)), units="m")
indep_var_comp.add_output("fuel_mass", val=50000 * 0.45, units="kg")  # 168k*0.45
...
# Loop through and add a certain number of aerostruct points. These are the cruise and maneuver points
for i in range(2):
...
recorder = om.SqliteRecorder("aerostruct_wb_baseline.db")
...
prob.model.add_design_var("wing.spar_thickness_cp", lower=0.003, upper=0.12, scaler=1e2)
prob.model.add_design_var("wing.skin_thickness_cp", lower=0.003, upper=0.12, scaler=1e2)
...
# Constraints
prob.model.add_constraint("AS_point_0.CL", equals=0.5)  # 0.82
